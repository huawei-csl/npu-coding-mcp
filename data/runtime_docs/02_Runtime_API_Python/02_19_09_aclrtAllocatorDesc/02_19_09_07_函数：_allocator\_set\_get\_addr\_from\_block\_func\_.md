...
使用用户提供的 Allocator 场景下，设置 " 根据申请来的 block 获取 device 内存地址 " 的回 调函数。
...
ret = acl.rt.allocator\_set\_get\_addr\_from\_block\_func\_to\_desc(allocatorDesc, func)

| 参数名           | 说明                                                                                                |
|---------------|---------------------------------------------------------------------------------------------------|
| allocatorDesc | int ， Allocator 描述符指针地址。需提前调用 acl.rt.allocator_create_desc 接口设置 Allocator 描述信息。                   |
| func          | ● 根据申请来的 block 获取 device 内存地址的回调函数。 回调函数定义如下： def allocator_get_addr_from_block_func(block): pass |
