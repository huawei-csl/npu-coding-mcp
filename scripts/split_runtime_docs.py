"""Split the monolithic Runtime API markdown into per-section files under data/runtime_docs/.

Usage:
    python scripts/split_runtime_docs.py /home/ebezati/CANN-9.0.0-Runtime-zh.md data/runtime_docs/
"""
...
def _section_to_dirname(section_number: str, title: str) -> str:
    ...
    short_title = title.replace("/", "_").replace("&amp;", "&").replace(" ", "_").replace("(", "").replace(")", "")
...
def _fix_merged_content(entries: list[dict]) -> list[dict]:
    """Fix Docling merging bug for consecutive API sections."""
    fixed: list[dict] = []
    i = 0
    BOILERPLATE_RE = re.compile(
        r"^## (功能说明|函数原型|参数说明|返回值说明|约束说明|"
        r"接口原型|流水类型|产品支持情况|注意事项|参考资源)\s*$"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Split Runtime API markdown into per-section files")
    parser.add_argument("input", type=Path, help="Path to CANN-9.0.0-Runtime-zh.md")
    parser.add_argument(
        "output",
        type=Path,
        nargs="?",
        default=Path("data/runtime_docs"),
        help="Output directory (default: data/runtime_docs)",
    )
