"""Load and query the Runtime documentation index."""
...
from npu_coding_mcp.runtime.models import ChapterInfo, SearchResult, SectionContent, SectionInfo


class RuntimeStore:
    """Access layer for the Runtime documentation index."""
    ...
    def search_api(self, api_name: str) -> list[SearchResult]:
        ...
        sql = """SELECT file_path, section_title, section_number, pdf_pages,
                 ...
                 FROM sections
                 WHERE sections MATCH ?
                   AND (file_path LIKE '01_Runtime_API_C/%' OR file_path LIKE '02_Runtime_API_Python/%')
                 ORDER BY rank
                 LIMIT 10"""
        results = []
        try:
    ...
    def list_sections(self, parent_path: Optional[str] = None) -> list[SectionInfo]:
        if parent_path:
            ...
            sql = """SELECT file_path, section_title, section_number, pdf_pages
                     FROM sections WHERE file_path LIKE ? ORDER BY file_path"""
            rows = self.conn.execute(sql, (prefix + "%",)).fetchall()
        else:
            sql = """SELECT file_path, section_title, section_number, pdf_pages
                     FROM sections ORDER BY file_path"""
    ...
    def list_chapters(self) -> list[ChapterInfo]:
        ...
        chapter_titles = {
            "01_Runtime_API_C": "Runtime API (C)",
            "02_Runtime_API_Python": "Runtime API (Python)",
        }

        chapter_map = {
            "01_Runtime_API_C": "Runtime C API — initialization, device/context/stream/event management, memory, kernel execution, model inference, profiling, data types",
            "02_Runtime_API_Python": "Runtime Python API — pyACL bindings mirroring the C API: device, stream, event, memory, kernel, profiling, data types",
        }
...
def load_store(docs_path: Path, index_path: Path) -> RuntimeStore:
    if not index_path.exists():
        raise FileNotFoundError(f"Index not found at {index_path}. Run 'build-runtime-index' first.")
    return RuntimeStore(docs_path, index_path)
